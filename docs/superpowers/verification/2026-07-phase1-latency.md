...
**Local vs. Render analysis:** Render medians run roughly 4-6x local medians (e.g. `/api/customer/home`: 210ms → 793ms), but the *ratio* is broadly consistent across all four endpoints rather than concentrated on one — this is the signature of fixed network/infra overhead (cross-region TLS handshake + Render free-tier compute, both hitting the same Supabase Postgres instance from different network paths) rather than a query-specific problem. No endpoint disproportionately spiked, no evidence of a missing index or N+1 query was observed at seed scale. If Render latency grows non-uniformly as data scales, that would be the signal to look at query plans; at seed scale it does not warrant one.
