...
signupForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  [nameInput, emailInput, phoneInput, passwordInput, vehicleInput].forEach(clearFieldError);
  ...
  if (!VEHICLES.includes(vehicleType)) { showFieldError(vehicleInput, "Choose a vehicle type."); hasError = true; }
  ...
  // The ID document is captured client-side only this phase (not uploaded);
  // the SPA reads role from /api/me after the token is saved.
  pendingSignup = { name, email, phone, password, role: "delivery_partner", vehicleType };
